...
class iRODSService:
    '''Encapsulates iRODS command line tools.
    '''
    ...
    def __init__(self, plugin, service_obj):
        ...
        self._cw    = None
        self._use_ssh = False # this should probably never be true...
    ...
    def _check_context(self):
        ...
        #################################################################
        ## irods:// URL
        if self._url.scheme in ["irods"]:
            self._use_ssh = False
            try:
                ## SET UP OUR COMMAND WRAPPER
                cw = CommandWrapper.initAsLocalWrapper(logger=self._pi)
                cw.connect()
                self._cw = cw

            except CommandWrapperException, ex:
                self._pi.log_error("Problem creating local iRODS command wrapper for %s: %s." \
                    % (self._url, ex))


## \/ OLE'S SGE STUFF BELOW \/ ##
